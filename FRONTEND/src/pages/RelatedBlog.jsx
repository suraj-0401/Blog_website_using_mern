...
function RelatedBlog() {
  ...
  useEffect(() => {
    const fetchBlog = async () => { 
      try {
        const response = await axios.get('http://localhost:3000/api/blogs/getRelatedBlog', {
          params: { category: 'fourth blog' } 
        }); 
        setBlogs(response.data); 
        console.log(response.data);
      } catch (error) {
        console.log(error);
        setError("Error occurred while fetching the blogs"); // Set error message
      }
    };
